...
export async function generateMetadata({ params }: Props) {
  ...
  return generatePageMetadata({
    title: `Student Jobs in ${city.name}, Germany | Plan Beta`,
    description: `Find Werkstudent positions, mini-jobs, and part-time work for international students in ${city.name}, Germany. Browse real openings and learn the German you need to land them.`,
    keywords: [
      `student jobs ${city.name}`,
      `werkstudent ${city.name}`,
      `mini job ${city.name} germany`,
      `part time ${city.name}`,
      `student jobs ${city.name} germany`,
      `werkstudent jobs ${city.name}`,
      `part time work ${city.name} germany`,
      `international student jobs ${city.name}`,
    ],
    path: `/jobs/student-jobs/${slug}`,
  })
}
...
export default async function StudentJobsCityPage({ params }: Props) {
  const { city: slug } = await params
  const city = getJobBoardCityBySlug(slug)
  if (!city) notFound()

  const cityIntro = CITY_NICHE_INTROS["student-jobs"][city.slug]
    ...
    <div className="min-h-screen bg-[#0a0a0a]">
      <BreadcrumbSchema
        items={[
          ...
          { name: "Student Jobs", url: "https://theplanbeta.com/jobs/student-jobs" },
          { name: city.name, url: `https://theplanbeta.com/jobs/student-jobs/${slug}` },
        ]}
      />
      ...
      {cityIntro && (
        <section className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-6 mb-8">
          <div className="bg-[#111] border border-white/[0.06] rounded-xl p-6">
            <h2 className="text-lg font-semibold text-white mb-2">Student Jobs in {city.name}</h2>
