...
export default function CfoPage() {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-3.5rem)] lg:h-screen">
      {/* Header */}
      <div className="flex-shrink-0 border-b border-white/[0.06] bg-[#0a0a0a]/80 backdrop-blur-sm px-4 sm:px-6 py-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-semibold text-white">CFO Agent</h1>
              <p className="text-xs text-gray-500">Live business data &middot; IPO strategy</p>
            </div>
          </div>
          {messages.length > 0 && (
            <button
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-4">
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full max-w-2xl mx-auto">
            ...
            <h2 className="text-lg font-semibold text-white mb-1">Talk to your CFO</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full">
              {SUGGESTED_QUESTIONS.map((q) => (
                <button
                  ...
                  className="text-left text-sm px-3 py-2.5 bg-white/[0.03] border border-white/[0.06] rounded-lg text-gray-400 hover:text-white hover:bg-white/[0.06] hover:border-white/[0.1] transition-all"
                >
                  {q}
                </button>
              ))}
                ...
                >
                  {msg.role === "assistant" && (
                    <div className="flex items-center gap-1.5 mb-2">
                      <span className="text-emerald-400 text-xs font-semibold">CFO</span>
      ...
      {/* Input */}
      <div className="flex-shrink-0 border-t border-white/[0.06] bg-[#0a0a0a]/80 backdrop-blur-sm px-4 sm:px-6 py-3">
        <div className="flex items-end gap-2 max-w-3xl mx-auto">
          <textarea
            ...
            rows={1}
            disabled={loading}
            className="flex-1 resize-none bg-white/[0.05] border border-white/[0.1] rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-gray-600 focus:ring-2 focus:ring-emerald-500/30 focus:border-emerald-500/50 transition-all disabled:opacity-50"
          />
          <button
            onClick={() => sendMessage()}
            disabled={!input.trim() || loading}
            className="flex-shrink-0 w-10 h-10 bg-emerald-600 hover:bg-emerald-500 disabled:bg-gray-700 disabled:opacity-50 rounded-xl flex items-center justify-center transition-colors"
          >
...
function formatCfoResponse(text: string) {
  return text.split("\n").map((line, i) => {
    ...
    if (line.startsWith("## ")) {
      return <h3 key={i} className="text-white font-bold mt-4 mb-1 text-base">{line.slice(3)}</h3>
    }
    ...
    // Tree-like lines
    if (line.match(/^[├└│─]/)) {
      return <p key={i} className="font-mono text-xs text-gray-400 leading-relaxed">{formatted}</p>
    }
